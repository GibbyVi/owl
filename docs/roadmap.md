...
owl checkup
  ...
  [HINT] run 'owl checkup --fix <field> [<field> ...]' to repair specific fields
  [HINT] see 'owl checkup --help' or check the docs for available fields
```

`owl checkup --fix` **requiere especificar los campos a arreglar**:

```
  [HINT] usage: owl checkup --fix <field> [<field> ...]
         fields: name, version, description, entry, profile,
                 opt-level, compiler, output, cache, sources, tests
         example: owl checkup --fix name version entry profile
```

Esto da control al desarrollador sobre qué campos se modifican. Solo los campos
especificados reciben valores por defecto si están vacíos; el resto se preservan
tal cual (incluso si están vacíos).
...
```
fn json_quote: (s :str) :str { return "\"" + s + "\"" }
```
...
load kioto::json   # Syntax Error at line 2 (blank line confuses parser)
```
...
but the backend's internal representation requires declarations to
precede usage in execution order.
